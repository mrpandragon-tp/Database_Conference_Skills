...
- 新增中英双语的“好数据库论文”定义，综合 80 篇全文 profile 和四份修订 PDF 中的 92 条导师批注。
- 定义跨阶段统一契约：重要问题、完整论文对象、核心机制、保证、证据和边界。
- 新增段落职责、符号预算、学习组件、可执行 Method、实验分析、简单语言、就地引用和非防御性写作检查。
- 将共享契约接入八个 skill，并更新对应回归测试。

## v0.4 多轮修稿经验更新

- 新增中英双语的 SIGMOD 风格多轮修稿经验。
- 新增导师批注修订协议，覆盖 topic sentence、段落职责、回复串合并与批注闭环。
- 为八个 skill 补充论文身份、消融归因、baseline 角色、形式一致性、双栏图表和 LaTeX 协作规则。
- 更新八个回归测试，覆盖新识别出的失败模式。
